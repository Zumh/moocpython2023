...
def older_book(book1: Book, book2: Book):
    ...
    """
    result = f"{book1.name} is older, it was published in {book1.year}"
    if book1.year > book2.year:
        result = f"{book2.name} is older, it was published in {book2.year}"
    elif book1.year == book2.year:
        result = f"{book1.name} and {book2.name} were published in {book1.year}"
    print(result)
