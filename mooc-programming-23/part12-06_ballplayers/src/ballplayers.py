...
def most_goals(ball_players: list[BallPlayer])->str:
    """
...
def most_points(ball_players: list[BallPlayer])->tuple:
    # number_of_goals = 0
    # number_of_assists = 0

    # total_point = number_of_goals + number_of_assists
    # player_most_point =  (name, shirt_number)
    player = max(ball_players, key= lambda player: player.goals + player.passes)
    return(player.name, player.number)

def least_minutes(ball_players: list[BallPlayer])->BallPlayer:
    """
